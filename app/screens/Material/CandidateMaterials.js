...
const ProductList = () => {
  ...
  const fetchCandidateMaterials = async () => {
    ...
    try {
      ...
      if (storedUserId) {
        const response = await axios.get(`${API_URL}/get-candidate-materials`);

        const updatedProducts = response.data.map((product) => {
          const picUrl = `${API_URL}/candidate-material-pic/${product.pic_document.cm_pic_id}.${product.pic_document.extension}`;
          return {
            ...product,
