...
const VariableOrder = () => {
  ...
  const fetchVariableOrder = async () => {
    setLoading(true);
    try {
      ...
      if (storedUserId) {
        const response = await axios.get(
          `${API_URL}/variable-orders/${cc_id.key}`
        );
          ...
          response.data.map(async (product) => {
            try {
              ...
              const imageResponse = await axios.get(
                `${API_URL}/get-candidate-material-pic/${product.cm_id}`
              );
              const picUrl = imageResponse.data?.image_url
                ? `${API_URL}/candidate-material-pic${imageResponse.data.image_url}`
                : null;
  ...
  useEffect(() => {
    fetchVariableOrder(); // Fetch products on component mount
  }, [cc_id]);
  ...
  const fetchUploadedImage = async (status, cr_id) => {
    try {
      ...
      const response = await axios.get(`${API_URL}/${link}/${cr_id}`);
      const imageUrls =
  ...
  return (
    <View style={styles.mainContainer}>
      ...
      <UploadImageModal
        ...
        onUploadSuccess={fetchVariableOrder}
